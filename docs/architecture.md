...
기존 인원의 현재 프로필을 입력 행에 보완한다. 이름·개인번호는 식별자를 대체하지 않는다.
충전액·비고·원문은 보존하되 이월은 다시 입력하며 연결 후 재검수를 거친다. 최종 동기화는
기존의 정규화된 8자리 포인트번호를 기준으로 한다. 번호는 외부 시스템이 발급하고 앱은
자동 생성하지 않는다. 신규 미발급 행은 초안에 남기며 발급 후 검증을 통과해야 확정한다.
