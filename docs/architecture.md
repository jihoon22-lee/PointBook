...
`BalanceRecord.profile_data/note/provenance/observed_at`에 기록의 상태·비고·근거를 저장하고
`BalanceRevision`에 원본과 정정판을 보존한다. 원본에 남은 인원 정보는 감사 자료이며,
일반 조회·집계·Excel의 이름·번호·팀·계급은 현재 인원 마스터를 사용한다. 월별 별도
인원 정보 수정은 제공하지 않는다. `BalanceAdjustment`는 현재 확인 잔액의
...
읽기 전용 `integrity.py` 검사는 계산식·관측 연결·현재값·FK·월/기록의 인원 정보 형식을 검사한다.
...
전후가 섞이지 않게 한다. HTML·XLSX 출력은 이 DTO를 사용한다. 합계는 Python 정수로
...
기본 요청서 v2는 포인트번호 없는 9열이며 v1의 10열 업로드도 유지한다. 제목 포함
붙여넣기는 번호 없는 5열 또는 v2 전체 9열을 지원하고 기존 번호 포함 입력을 보존한다.
번호 없는 행은 이름·개인번호·계정 유형이 모두 정확히 일치하는 기존 계정이 하나면 자동
연결한다. 개인번호가 빈 공용계정과 여러 계정이 일치하는 경우에는 자동 연결하지 않는다.
자동 연결은 포인트번호만 보완하고 요청서 팀·계급·충전액·비고·원문·이월을 보존한다.
연결 대상이나 이름·개인번호가 바뀌면 기존 이월을 다시 입력한다. 자동 연결 결과도 수동 변경한다.
`POST /monthly/link`는 이름 또는 개인번호가 일치하는 후보 중 관리자가 명시적으로 선택한
기존 인원을 연결한다. 입력 프로필을 덮어쓰지 않고 현재 마스터와 이번 입력의 이름·개인번호·
팀·계급·유형 차이를 해당 칸 아래 위아래 두 줄로 제시한다. 자동 연결도 같은 차이 선택 절차를 거친다. 선택 상태는
초안에 보존하고 확정 시 선택값으로 현재 마스터를 갱신한다. 이번 입력의 빈 팀·계급을
명시적으로 선택한 경우도 현재 값 삭제로 반영한다. 유형 변경은 인원 편집 경계를 유지하며
기존 유형 선택은 허용한다. 수동 연결은 충전액·비고·원문을 보존하되 이월을 초기화하고
재검수를 거친다. 신규 선택은 외부 번호 발급을 대신하지 않으며 등록된 번호의 신규 선언은
거부한다. 화면은 연결 상태 태그·색상과 차이 선택을 제공하고 재검수·연결 뒤 스크롤과
입력 위치를 복원한다. 화면 열 순서는 Excel v2 파일 계약과 분리한다. 최종 동기화는 정규화된 8자리 포인트번호를
기준으로 한다. 이름·개인번호는 고유 식별자가 아니며 중복 계정을 합치지 않는다.
번호는 외부 시스템이 발급하고 앱은 자동 생성하지 않는다. 신규 미발급 행은 초안에 남기며
발급 후 검증을 통과해야 확정한다.

`xlsx.py`는 표준 양식 버전·셀 위치를 검증하고 공통 RawRequestRow→검수·초안으로 전달한다.
ZIP/XML 크기·항목·행/열·시간, defusedxml, 외부 관계·매크로 차단과 실행 슬롯 2개를 적용한다.
입력/보고서는 다른 양식이며 수식은 평가하지 않는다. 출력은 Report의 조회 범위·변경 이력 기준·정렬을
공유하고 텍스트 셀 타입을 명시한다. 16자리 이상 정수 합계는 Excel 정밀도 손실을 피하려 문자열로 보존한다.
`RawRequestRow`와 금액 수정은 비고의 CRLF/CR을 LF로 통일한다. 검수·정정 화면의 비고는
`textarea`로 입력받아 업로드·초안·재검수·확정·Excel까지 줄바꿈과 빈 줄을 보존한다.
상세 한도·오류 계약은 [Excel 안내](excel-workflow.md), 전체 수용 근거는 [v1.4.0 수용 기록](v1.4.0-acceptance.md)에 있다.


### 기존 월별 장부와 인원 정보 (실기 피드백 반영)

기존 Excel 출처(`master_at_migration`, `legacy_import`)와 과거 월 추가
(`reference_at_correction`)의 일반 인원 상태는 해당 월 지급액으로
읽는다(0원 비재직 / 지급액 있음 재직). 원본 금액·불변 정정판 JSON을 덮어쓰지 않고, 선택한
정정판의 지급액에 같은 규칙을 적용한다. 신규 이관도 이 규칙으로 상태를 보존한다. 공용계정은
항상 active이고, 웹 월간 확정에서 명시적으로 확인한 상태는 해당 기록을 따른다.
과거 월 추가의 지급액 판정은 월별 상태에 적용하며 현재 Person.status를 바꾸지 않는다.
...
월간 재직/비재직 전환 수는 월간 기록 건수와 별도다. 실제 월간 확정(`observed`) 기록은
`LedgerOperation`의 확정 내역과 원래 포인트번호로 연결해 실제 전환을 우선 판단한다. 따라서 첫 개별 등록이나
개별 복귀 후 요청서에서 빠진 인원도 비재직 전환으로 센다. 기존 장부·과거 삽입 기록은
선택한 정정판의 직전 실제 월간 기록과 상태를 비교한다. 미관측 달·잔액 보정·기존 비재직·
공용을 신규 비재직 전환으로 세지 않으며 이후 번호 변경으로 이미 발생한 전환을 잃지 않는다.


## v1.5.0 입력·재직 기간·팀 집계

- `sync.ABSENT_ACTIONS`는 신규 비재직 전환과 `inactive_kept`를 구분한다. 두 구역 모두
  기존 `deactivated_carry_<point_no>` 폼/초안 map을 사용하며 요청서 인원에서 제외한다.
  기존 비재직 기록은 0원 충전·비재직 상태와 실제 이월 관측을 저장하고 전환 집계를 늘리지 않는다.
- `review.canonical_money/canonical_row`는 승인과 재전송 비교에서 유효 금액의 표시 차이를
  정규화한다. 입력·초안의 오류 원문은 그대로 보존한다.
- `tenure.tenure_labels`는 월별 상태와 명시적 상태 변경 근거를 일괄 조회해 기간을 계산한다.
  시작·종료월 포함, KST 현재 월까지 진행 구간, 근거 없는 비재직 추정 금지와 팀 이동 독립을 유지한다.
- `stats.Report.active_teams`는 선택 범위의 재직 표, `holding_teams`는 as_of 전체 표다.
  동일 operation cutoff·필터·현재 팀을 사용하고 화면과 Excel이 두 집계를 공유한다.
- `money-input.js`는 금액 입력만 쉼표 표시하고 식별자를 건드리지 않는다. `review-header.js`는
  원본 표 폭·가로 위치·상단 메뉴 높이에 동기화한 표시용 제목을 제공한다.
  원본 의미 구조를 유지하도록 복제 제목은 접근성 트리에서 제외한다.


## v1.5.1 재검토 보완

- 비재직 이월은 기존 raw map과 별도로 서명된 입력 대상 연결을 저장한다. 현재 인원 ID·포인트번호·
  이름·개인번호·유형·월·재직 구분이 일치할 때만 적용한다. 불일치·구버전 입력은 서명된 보존 목록으로
  분리해 초안 저장·복원·오류 재제출에서 잃지 않고, 현재 대상에는 다시 입력받는다.
- `tenure_labels`는 월간 작업 ID와 최초 `BalanceRevision`의 `record.person_id`로 당시 대상을
  연결한다. 후속 번호 재사용·과거 삽입·정정은 원래 연결을 바꾸지 않는다. 최근 상태 불일치는
  완료된 과거 재직 기간을 지우지 않는다.
- 재직 팀 통계는 `activity_profile`과 `balance_profile`을 독립적으로 필터링한다.
  유형 미확인 항목에도 현재 팀 필터를 적용한다. 조회 월과 마지막 잔액 기준 월은 별개로 유지한다.
- `FieldError.field`와 `Review.error_fields`가 오류 문구와 실제 폼 입력 칸을 분리한다.
  월간·초안·CSRF 파서는 `MAX_REVIEW_FORM_FIELDS` 한도를 공유한다.
- `kst_datetime`은 업무 화면에서 UTC 저장 시각을 한국 시간으로 표시한다. 원본 이력은 수정하지 않는다.
  자동 저장은 주소가 달라졌을 때만 `history.replaceState`를 호출한다.
