...
- 포인트번호 없는 월간 요청서는 Excel v2 또는 제목 포함 붙여넣기로 받는다. 이름·개인번호·
  ...
  이후 연결 대상이나 이름·개인번호가 바뀌면 기존 이월을 다시 입력한다. 수동 연결은 현재 마스터
  정보를 보완하고 충전액·비고·원문을 보존하되 이월은 다시 입력한다. 신규 번호 미발급 행은
  초안으로 남기고 실제 번호 발급 후 검수·확정한다. 기존 v1과 번호 포함 입력 호환을 유지한다.
