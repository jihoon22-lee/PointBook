...
- 구분자를 제거한 **8자리 포인트번호**가 업무 식별자다. 선행 0을 보존하고,
  이름·개인번호의 중복을 허용한다. 포인트번호는 외부 시스템이 발급하며 PointBook에서 자동 생성하지 않는다.
- 포인트번호 없는 월간 요청서는 Excel v2 또는 제목 포함 붙여넣기로 받는다. 이름·개인번호·
  계정 유형이 모두 정확히 일치하는 기존 계정이 하나면 자동 연결하며, 개인번호가 빈 공용계정은
  자동 연결하지 않는다. 같은 이름·개인번호의 여러 계정은 수동 선택하고 연결 결과를 바꿀 수 있다.
  자동 연결은 포인트번호만 보완하며 요청서 팀·계급·충전액·비고·원문·이월을 보존한다.
  이후 연결 대상이나 이름·개인번호가 바뀌면 기존 이월을 다시 입력한다. 수동 연결은 요청서
  정보를 덮어쓰지 않고 현재 값·이번 입력값을 해당 칸 아래 위아래 두 줄로 보여 차이 항목을 명시적으로 선택한다.
  자동 연결에도 같은 차이 선택을 적용한다. 선택 결과는 초안에 보존하고 확정 때 현재 마스터에
  반영한다. 이번 입력의 빈 팀·계급을 명시적으로 선택하면 현재 값을 비울 수 있다. 유형 변경은
  기존 인원 편집에서 처리하며 월간 검수에서는 기존 유형을 선택해 유지할 수 있다.
  충전액·비고·원문을 보존하고 수동 연결 때 이월은 다시 입력한다. 신규 번호 미발급 행은
  초안으로 남기고 실제 번호 발급 후 검수·확정한다. 등록된 포인트번호로 신규를 선언하면 거부한다.
  검수 화면은 자동 연결·직접 연결·연결 확인·신규를 태그와 색상으로 구분하며 이름·개인번호·
  포인트번호·팀·계급·이월 잔액·충전액·비고·유형 순으로 표시한다. 화면 열 변경을 Excel v2의
  열 계약에 반영하지 않는다. 기존 v1과 번호 포함 입력 호환을 유지한다.
...
- AI 인식은 제안이며 사람이 검수·수정 후 확정한다. 재직 유지·복귀·비재직 전환·신규를
  포함한 처리 대상과 기존 비재직 일반 인원의 이월 잔액을 각각 한 번 입력받는다.
  기존 비재직자는 별도 구역에서 입력하며 0원 충전·비재직 상태를 유지하고 신규 전환 인원에 세지 않는다.
