...
def _report_workbook(
    ...
    *,
    ...
    summary_values = [
        ...
        ("당시 유형·팀 분류 불가", len(report.unclassified_rows)),
        (
            "월간 충전 합계",
        ...
        (
            "범위 안내",
            "월간 활동과 잔액은 각 실제 관측 당시 소속/유형에 귀속됩니다. 미확인 잔액은 합계에서 제외합니다.",
        ),
        (
            "출처 안내",
            "기존 장부의 당시 정보·원래 작성자/시각을 보증하지 않습니다. 각 행 출처를 확인하세요.",
        ),
    ...
    for title, rows in [("인원", report.rows), ("분류 미확인", report.unclassified_rows)]:
        ...
        for index, row in enumerate(rows, 2):
            ...
            values = (
                ...
                row.activity_note,
                row.note,
                (row.activity_profile or {}).get("team_name"),
